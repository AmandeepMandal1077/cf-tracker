        ...
        for (const contestLink of contestLinks) {
            try {
                ...
                if (resJson.status !== "OK") {
                    console.error(
                        `Failed to fetch standings for contest ${contestId}`,
                    );
                    break;
                }
                ...
                console.error(
                    `Error processing contest ${contestLink}:`,
                    error,
                );
            }
        }
...
const getQuestionInfo = async (url: string) => {
        ...
        const fullStatement = Array.from(problem.children).map(
            (child, index) => {
                ...
                if (index == 1 || index == 5) {
                    child
                        .querySelectorAll(
                            ".MathJax, .MathJax_Preview, .section-title",
                        )
    ...
    memoryLimit =
        memoryLimit?.replace("memory limit per test", "").trim() || "";
    ...
    const problemDetails = {
        ...
        inputStatementFormatted: codeforcesDescriptionFormat(
            obj.inputStatement,
        ),
        outputStatementRaw: obj.outputStatement,
        outputStatementFormatted: codeforcesDescriptionFormat(
            obj.outputStatement,
        ),
